# Google Colab용 타이어 공기압 예측 LSTM 모델
...
from tensorflow.keras.layers import Input, LSTM, Dense, Dropout, Concatenate, BatchNormalization, Bidirectional
...
class TirePressureLSTMPredictor:
    ...
    def build_model(self, input_shape_raw, input_shape_features):
        """LSTM 모델 구축 (32-64 psi 범위로 제한)"""
        # 원시 데이터 입력 브랜치 (LSTM)
        raw_input = Input(shape=input_shape_raw, name='raw_input')
        
        # Bidirectional LSTM layers
        x1 = Bidirectional(LSTM(64, return_sequences=True, dropout=0.2, recurrent_dropout=0.2))(raw_input)
        x1 = BatchNormalization()(x1)
        
        x1 = Bidirectional(LSTM(32, return_sequences=True, dropout=0.2, recurrent_dropout=0.2))(x1)
        x1 = BatchNormalization()(x1)
        
        x1 = Bidirectional(LSTM(16, dropout=0.2, recurrent_dropout=0.2))(x1)
        x1 = BatchNormalization()(x1)
    ...
    def train(self, X_raw, X_features, y, validation_split=0.1, epochs=100, batch_size=32):
        ...
        # 콜백 설정
        callbacks = [
            EarlyStopping(monitor='val_loss', patience=20, restore_best_weights=True),  # LSTM은 더 오래 훈련
...
def main():
    ...
    # 모델 초기화
    predictor = TirePressureLSTMPredictor(sequence_length=1000, step_size=100)
    ...
    # 모델 저장
    predictor.model.save('tire_pressure_lstm_model.h5')
    print("\nModel saved as 'tire_pressure_lstm_model.h5'")
    
    # 모델 다운로드
    print("\n모델 파일을 다운로드하시겠습니까?")
    download_choice = input("y/n: ").lower()
    if download_choice == 'y':
        files.download('tire_pressure_lstm_model.h5')
...
print("\n=== LSTM 모델 특징 ===")
print("✅ Bidirectional LSTM으로 양방향 시계열 패턴 학습")
print("✅ 시간적 의존성 모델링에 특화")
print("✅ 긴 시퀀스에서의 장기 의존성 학습")
print("✅ 드롭아웃과 배치 정규화로 과적합 방지") 